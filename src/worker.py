...
from src.tiktokautouploader.function import upload_tiktok, login_only
...
class TikTokUploadMessage(BaseModel):
    ...
    tiktok_username: Optional[str] = Field(default=None, description="Имя пользователя TikTok для входа.")
    tiktok_password: Optional[str] = Field(default=None, description="Пароль от аккаунта TikTok для входа.")
...
class TikTokCookieMessage(BaseModel):
    account_name: str = Field(..., description="Имя аккаунта TikTok для сохранения cookie.")
...
async def handle_tiktok_login(msg: TikTokLoginMessage):
    ...
    """
    logger.info(f"Received login task for account: {msg.account_name} with password {msg.tiktok_password}")
...
async def handle_tiktok_cookies(msg: TikTokCookieMessage):
    ...
    try:
        ...
        logger.success(f"Successfully saved and normalized cookies for account: {msg.account_name}")
        ...
        logger.error(f"Failed to parse cookies JSON string for account {msg.account_name}.")
        status_message = CookieStatusMessage(
            tg_user_id=msg.tg_user_id,
            account_name=msg.account_name,
            success=False,
        ...
        logger.exception(f"An error occurred while processing cookies for {msg.account_name}: {e}")
        status_message = CookieStatusMessage(
            tg_user_id=msg.tg_user_id,
            account_name=msg.account_name,
            success=False,
    ...
    logger.info(f"Sent cookie processing status for account {msg.account_name} back to the bot.")
